...
public class DotTreeGUI extends DrawingGUI {
	...
	private int mouseRadius = 20;					// circle around mouse location, for querying
	...
	public void handleMousePress(int x, int y) {
		if (mode == 'a') {
			...
			// reset found so that old query points are deleted after query is over
			found = null;
			
			// if there is no tree yet, this is the first mouse press and it should initialize the tree
			if(tree == null) 
				tree = new PointQuadtree(new Dot(x,y), 0, 0, width, height);
			// otherwise, tree already exists, so mouse press requires inserting new point into tree
			else 
			...
			// initiate query, find points within circle of mouse using tree
	...
	/**
	 * this is my own test tree
	 */
	private void test2() {
		found = null;
		Dot a = new Dot(100,100);
		Dot b = new Dot(50,50);
		Dot c = new Dot(450,50);
		Dot d = new Dot(50,350);
		Dot e = new Dot(450,350);
		Dot f = new Dot(275,225);
		Dot g = new Dot(365,285);
		tree = new PointQuadtree<Dot>(a, 0,0,800,600); // this root A
		tree.insert(b); // B
		tree.insert(c); // C
		tree.insert(d); // D
		tree.insert(e); // E
		...
		tree.insert(g); // G
		int bad = 0;
		List<Dot> points_found;
		
		// size of ArrayList of all points should be 7
		
		if(tree.allPoints().size( ) != 7)
			bad++;
		
		// part 1
		// query points in vicinity of root A
		// search radius of 70 should identify point A only
		// search radius of 71 should hit both points A and B
		
		points_found = tree.findInCircle(100, 100, 70);
		if(!(points_found.contains(a)))
			bad++;
		
		points_found = tree.findInCircle(100, 100, 71);
		if(!(points_found.contains(a) && points_found.contains(b)))
			bad++;
		
		// part 2
		// query points near point G
		// search radius of 50 should identify point G only
		// search radius of 200 should hit points G, E, and F
		
		points_found = tree.findInCircle(365, 285, 50);
		if(!(points_found.contains(g)))
			bad++;
		
		points_found = tree.findInCircle(365, 285, 200);
		if(!(points_found.contains(g) && points_found.contains(e) && points_found.contains(f)))
			bad++;
		
		// part 3
		// query points around point g with search radius 500
		// this search radius should be large enough to find all points in canvas
		
		points_found = tree.findInCircle(365, 285, 400);
		if(!(points_found.contains(a) && points_found.contains(b) 
				&& points_found.contains(c) && points_found.contains(d) 
				&& points_found.contains(e) && points_found.contains(f)
				&& points_found.contains(g)))
			bad++;
	...
	public void drawTree(Graphics g, PointQuadtree<Dot> tree, int level) {
		...
		// identify and recurse for every child of root, using getChildren helper method
		List<PointQuadtree<Dot>> children = tree.getChildren();
		for(PointQuadtree<Dot> child : children)
			// draw PointQuadtree associated with the child 
			// increase level to indicate new color for this set of children
